...
class POP3 {
  ...
  connect(host, encrypt, port) {
    return new Promise((resolve, reject) => {
      if (!host) return reject({code: 401, message: errCode[401]})
  ...
  _sendCommand(verb, param) {
    ...
    return new Promise((resolve, reject) => {
      if (!verb) return reject({code: 301, message: errCode[301], verb: verb})
      ...
      this._sockt.on('data', _data => {
        ...
        if (this.respEnd) {
          return this._checkResp(resp, resolve, reject, verb)
        }
      })
      this._sockt.on('error', error => {
        logger.warn(error)
        return reject({code: 302, message: errCode[302], verb: verb})
      })
    })
  }

  _checkResp(resp, resolve, reject, verb) {
    if (resp.substr(0, 3).toUpperCase() === '+OK') {
      //命令执行成功
      return resolve(resp)
    } else if (resp.substr(0, 4).toUpperCase() === '-ERR') {
      //命令执行失败
      return reject({code: 303, message: errCode[303], verb: verb})
    } else {
      //未知结果返回
      return reject({code: 303, message: errCode[303], verb: verb})
    }
